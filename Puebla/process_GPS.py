...
path='/Users/dmelgar/Puebla2017/GPS/'
station_file=path+'station_info/gps1Hz.sta'
#station_file=path+'station_info/gps5Hz.sta'
gps_files=glob(path+'raw/1Hz/kin*')
#gps_files=glob(path+'raw/5Hz/kin*')
time_epi=UTCDateTime('2017-09-19T18:14:40')
epicenter=array([-98.72,18.40,57])  
...
if gps2sac:
    stanames=genfromtxt('/Users/dmelgar/Puebla2017/GPS/station_info/gps1Hz.sta',usecols=0,dtype='S')
    coords=genfromtxt('/Users/dmelgar/Puebla2017/GPS/station_info/gps1Hz.sta',usecols=[1,2])
    for k in range(len(gps_files)):
        ...
        if len(gap_positions)>0:  #There are gaps
            for i in range(len(gap_positions)):
                if i==0:        
                    ...
                    #What is first epoch?
                    time=UTCDateTime('2017-09-19T00:00:00')+timedelta(seconds=t[0])-timedelta(seconds=18) #Apply GPS->UTC leapseconds
                    #Apply start time
...
if cut_filter:
    ...
    for k in range(len(stanames)):
        try:
            ...
            #Write to file
            n.write(path+'trim1Hz/'+sta+'.LXN.sac',format='SAC')
            e.write(path+'trim1Hz/'+sta+'.LXE.sac',format='SAC')
            u.write(path+'trim1Hz/'+sta+'.LXZ.sac',format='SAC')
            #n.write(path+'wphase/'+sta+'.LXN.sac',format='SAC')
            #e.write(path+'wphase/'+sta+'.LXE.sac',format='SAC')
            #u.write(path+'wphase/'+sta+'.LXZ.sac',format='SAC')
        except:
            print 'Error on '+sta
...
if make_plots:
    ...
    for k in range(len(stanames)):
        ...
        try:
            ...
            n=read(path+'trim1Hz/'+sta+'.LXN.sac')
            e=read(path+'trim1Hz/'+sta+'.LXE.sac')
            z=read(path+'trim1Hz/'+sta+'.LXZ.sac')
            plt.figure()
            plt.subplot(311)
            ...
            plt.grid()
            plt.ylabel('Up (m)')
            plt.savefig(path+'plots/'+sta+'_long.png')
        except:
            print 'Error on '+sta
