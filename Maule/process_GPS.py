...
D.Melgar
01/2015
Pre-process Maule GPS data
'''



from glob import glob
from numpy import genfromtxt,where,array,float64,diff,mean,interp,arange
...
path='/Users/dmelgar/Maule2010/GPS/'
...
if move_mudpy:
    pathout='/Users/dmelgar/Slip_inv/maule/data/waveforms/'
    time_epi=UTCDateTime('2010-02-27T06:34:14')
    tcut=timedelta(minutes=5)
    ...
    for k in range(len(files)):
        ...
        if n[0].stats.delta>1.0:
            tmp=interp(arange(0,n[0].times()[-1]+1),n[0].times(),n[0].data)
            n[0].data=tmp
            tmp=interp(arange(0,e[0].times()[-1]+1),e[0].times(),e[0].data)
            e[0].data=tmp
            tmp=interp(arange(0,u[0].times()[-1]+1),u[0].times(),u[0].data)
            u[0].data=tmp
            n[0].stats.delta=1.0
            e[0].stats.delta=1.0
            u[0].stats.delta=1.0
        ...
        #Write to file
        n.write(pathout+sta+'.disp.n',format='SAC')
        e.write(pathout+sta+'.disp.e',format='SAC')
        u.write(pathout+sta+'.disp.u',format='SAC')
        
if make_pgd:
    pathout='/Users/dmelgar/PGD/GPS/sac/Maule2010/'
    time_epi=UTCDateTime('2010-02-27T06:34:14')
