...
data_path='/Users/dmelgar/PGD/GPS/sac/Melinka2016'
earthquakes=glob(path+'/station_info/*')

suffix='all'
pgd_folders=['/Users/dmelgar/PGD/GPS/sac/Melinka2016',]


Sspeed=3000 #S-wave mask
tmax=timedelta(seconds=90)
minpgd=0.05 #in m
maxdist=600 #in km
minsta=6
Nboot=1000
#hypo=[-73.951,-43.416,35.2] #USGS
hypo=[-74.391,-43.517,30.0] #CSN
...
if get_pgd:
    ...
    print hypo_time
    #Read station data
    stanames=genfromtxt(path+'station_info/melinka.sta',usecols=0,dtype='S')
...
if plot:
    ...
    t1,M1=view.source_time_function(u'/Users/dmelgar/Slip_inv/Melinka_usgs/output/inverse_models/models/gps_sm_vr3.0_laplace.0011.inv',[-73.951,-43.416,35.2])
    event='Melinka2016'
    M=genfromtxt('/Users/dmelgar/PGD/PGD/magnitudes/Melinka2016.mag')
    ...
    plt.errorbar(M[:,0],M[:,1],yerr=[merr,merr],fmt='o',color='#FF6347',ecolor='#FF6347',markersize=3,label=r'Mw')
    #plt.errorbar(M[:,0],M[:,2],yerr=[merr,merr],fmt='o',color='#6495ED',ecolor='#6495ED',markersize=3,label=r'With vertical')
    #plt.legend()
    plt.ylim([5,8.5])
    plt.xlim([0,90])
    plt.grid()
    plt.title('PGD magnitude, CSN hypo, 2016 Mw7.6 Melinka earthquake',fontsize=16)
    plt.ylabel(r'$M_w$',fontsize=16)
    plt.xlabel(r'Seconds after origin time',fontsize=16)
    plt.annotate(r'$2\times10^{19}$Nm/s',xy=(11.6,8.06))
    plt.plot([0,90],[7.6,7.6],'--')
    plt.fill(t1,3*(M1/M1.max())+5,'b',alpha=0.5)
    plt.subplots_adjust(bottom=0.17,left=0.1,right=0.95)
    plt.show()
