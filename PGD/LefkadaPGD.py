...
data_path='/Users/dmelgar/PGD/GPS/sac/Lefkada2015'
earthquakes=glob(path+'/station_info/*')

suffix='all'
pgd_folders=['/Users/dmelgar/PGD/GPS/sac/Lefkada2015',]
...
if get_pgd:
    ...
    #Read station data
    stanames=genfromtxt(path+'station_info/lefkada.sta',usecols=0,dtype='S')
    station_coords=genfromtxt(path+'station_info/lefkada.sta',usecols=[1,2])
    ...
    for ksta in range(len(stanames)):
        ...
        try:
            ...
            if n[0].stats.npts>0:
                ...
                time_to_all=0
                time_to_nov=0
                f=open(path+'PGD/Lefkada2015/'+stanames[ksta]+'.pgd','w')
...
if get_magnitude:
    ...
    for kt in range(len(tpgd)):
        ...
        if len(dnov)>=minsta:
            ...
            Mw_nov[kt]=scaling.PGD(dnov*100,R,coefficients=coeff_novert,weight=False,norm=norm)
            Mw_all[kt]=scaling.PGD(dall*100,R,coefficients=coeff_all,weight=False,norm=norm)
...
if plot:
    event='Lefkada015'
    M=genfromtxt('/Users/dmelgar/PGD/PGD/magnitudes/Lefkada2015.mag')
    plt.figure()
    ...
    plt.ylim([5,7.5])
    plt.grid()
    plt.title('PGD magnitude, 2015 Mw6.5 Lefkada earthquake',fontsize=16)
