...
data_path='/Users/dmelgar/PGD/GPS/sac/Michoacan1985'
earthquakes=glob(path+'/station_info/*')

suffix='all'
pgd_folders=['/Users/dmelgar/PGD/GPS/sac/Michoacan1985/']


Sspeed=3000 #S-wave mask
tmax=timedelta(seconds=200)
minpgd=0.05 #in m
maxdist=400 #in km
minsta=1
Nboot=1000
hypo=[-102.57,18.18,17]
...
if get_pgd:
    ...
    print hypo_time
    #Read station data
    stanames=genfromtxt(path+'station_info/michoacan.sta',usecols=0,dtype='S')
    station_coords=genfromtxt(path+'station_info/michoacan.sta',usecols=[1,2])
    ...
    for ksta in range(len(stanames)):
        ...
        try:
            ...
            if n[0].stats.npts>0:
                ...
                time_to_all=0
                time_to_nov=0
                f=open(path+'PGD/Michoacan1985/'+stanames[ksta]+'.pgd','w')
...
if plot:
    ...
    t1,M1=viewFQ.source_time_function(u'/Users/dmelgar/Slip_inv/Michoacan_main/output/ruptures/michkinematic.0000.rupt',[-102.57,18.18,17],dt=0.5)
    M1=M1/1e19
    event='Michoacan1985'
    M=genfromtxt('/Users/dmelgar/PGD/PGD/magnitudes/Michoacan1985.mag')
    ...
    plt.xlim([0,180])
    plt.grid()
    plt.title('1985 Mw8.0 Michoacan earthquake',fontsize=16)
    plt.ylabel(r'PGD magnitude',fontsize=16)
    plt.xlabel(r'Seconds after origin time',fontsize=16)
    plt.plot([0,180],[8.05,8.05],'--',lw=2)
    ax2 = ax1.twinx()
    ax2.fill(t1,3*(M1/M1.max())+5,'b',alpha=0.3)
