...
path='/Users/dmelgar/IquiqueAfter2014/'
gps_files=glob(path+'GPS/raw/difkin*')
time_epi=UTCDateTime('2014-04-03 02:43:13 ')
epicenter=array([-70.493,-20.571,22.4])
tmin=timedelta(seconds=20) #Keep these many seconds before p-wave
...
if gps2sac:
    stanames=genfromtxt('/Users/dmelgar/Iquique2014/station_info/iquique_after.lla',usecols=0,dtype='S')
    coords=genfromtxt('/Users/dmelgar/Iquique2014/station_info/iquique_after.lla',usecols=[1,2])
    for k in range(len(gps_files)):
        ...
        if len(gap_positions)>0:  #There are gaps
            for i in range(len(gap_positions)):
                if i==0:        
                    ...
                    #What is first epoch?
                    time=UTCDateTime('2014-04-03T00:00:00')+timedelta(seconds=t[0])-timedelta(seconds=16) #Apply GPS->UTC leapseconds
                    #Apply start time
        ...
        n.write(path+'GPS/proc/'+sta+'.LXN.sac',format='SAC')
        e.write(path+'GPS/proc/'+sta+'.LXE.sac',format='SAC')
        u.write(path+'GPS/proc/'+sta+'.LXZ.sac',format='SAC')
        #Re-read
        n=read(path+'GPS/proc/'+sta+'.LXN.sac')
        e=read(path+'GPS/proc/'+sta+'.LXE.sac')
        u=read(path+'GPS/proc/'+sta+'.LXZ.sac')
        #get positions
...
if cut_filter:
    ...
    for k in range(len(stanames)):
        ...
        #Write to file
        n.write(path+'GPS/trim/'+sta+'.LXN.sac',format='SAC')
        e.write(path+'GPS/trim/'+sta+'.LXE.sac',format='SAC')
        u.write(path+'GPS/trim/'+sta+'.LXZ.sac',format='SAC')
...
if make_plots:
    ...
    for k in range(len(stanames)):
        print stanames[k]
        sta=stanames[k]
        n=read(path+'GPS/trim/'+sta+'.LXN.sac')
        e=read(path+'GPS/trim/'+sta+'.LXE.sac')
        z=read(path+'GPS/trim/'+sta+'.LXZ.sac')
        plt.figure()
        plt.subplot(311)
        ...
        plt.grid()
        plt.ylabel('Up (m)')
        plt.savefig(path+'GPS/plots/'+sta+'.pdf')
