from numpy import genfromtxt,array,ones,zeros,r_,where,arange,mean
...
#hypocenter
epi=array([-94.11,14.85])

fout='/Users/dmelgar/Chiapas2017/strong_motion/synthesis/ground_motions_20s_highpass.txt'

#read station list
sta=genfromtxt('/Users/dmelgar/Chiapas2017/strong_motion/station_info/all_stations.txt',usecols=1,dtype='S')
lonlat=genfromtxt('/Users/dmelgar/Chiapas2017/strong_motion/station_info/all_stations.txt',usecols=[2,3])

#where's teh data?
path=u'/Users/dmelgar/Chiapas2017/strong_motion/sac/'
...
for k in range(len(sta)):
    ...
    try:
        z=read(path+sta[k]+'.HLE.sac')
    except:
        z=read(path+sta[k]+'.HNE.sac')
    
    n[0].data=n[0].data-mean(n[0].data[0:200])
    e[0].data=e[0].data-mean(e[0].data[0:200])
    z[0].data=z[0].data-mean(z[0].data[0:200])
...
#write to file
f=open(fout,'w')
f.write('# Ground motions for the M8.2 Tehuantepecs EQ. rotd50 SAs are computed at 21 frequencies (in Hz) they are:\n# 0.010,0.020,0.030,0.050,0.075,0.10,0.15,0.20,0.25,0.30,0.40,0.50,0.75,1.0,1.5,2.0,3.0,4.0,5.0,7.5,10.0\n') 
f.write('#\n')
