...
path='/Users/dmelgar/ElMayor2010/'
...
if gps2sac:
    stanames=genfromtxt('/Users/dmelgar/ElMayor2010/station_info/elmayor.sta',usecols=0,dtype='S')
    coords=genfromtxt('/Users/dmelgar/ElMayor2010/station_info/elmayor.sta',usecols=[1,2])
    for k in range(len(gps_files)):
        ...
        day=gps[:,0]
        icut=where(day==55290)[0]
        gps=gps[icut,:]
        t=gps[:,1]
        dt=round(diff(t).min(),1)
        print 'dt='+str(dt)
        gap_positions=where(diff(t)>(dt+dt/2))[0]+1
        ...
        if len(gap_positions)>0:  #There are gaps
            for i in range(len(gap_positions)):
                if i==0:        
                    ...
                    n[0].data=gps[0:gap_positions[0],3]/100 #It's in cm, convert to m
                    ...
                    u+=Trace()
                    #Fill with data
                    n[i].data=gps[gap_positions[i-1]:gap_positions[i],3]/100 #It's in cm, convert to m
            ...
            u+=Trace()
            #Fill with data
            n[i+1].data=gps[gap_positions[i]:,3]/100 #It's in cm, convert to m
            ...
            n[0].data=gps[:,3]/100 #It's in cm, convert to m
            e[0].data=gps[:,2]/100
            u[0].data=gps[:,4]/100
            #What is first epoch?
            time=t0+timedelta(seconds=t[0])-timedelta(seconds=15) #Apply GPS->UTC leapseconds
...
if make_pgd:
    pathout='/Users/dmelgar/PGD/GPS/sac/ElMayor2010/'
