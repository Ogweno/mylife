...
path='/Users/dmelgar/Ixtepec2017/GPS/'
station_file=path+'station_info/gps5Hz.sta'
#station_file=path+'station_info/gps5Hz.sta'
gps_files=glob(path+'difkin/5Hz/difkin*')
#gps_files=glob(path+'raw/5Hz/kin*')
time_epi=UTCDateTime('2017-09-23T12:53:02')
epicenter=array([-94.90,16.48,10.0])  
...
if gps2sac:
    stanames=genfromtxt('/Users/dmelgar/Ixtepec2017/GPS/station_info/gps5Hz.sta',usecols=0,dtype='S')
    coords=genfromtxt('/Users/dmelgar/Ixtepec2017/GPS/station_info/gps5Hz.sta',usecols=[1,2])
    for k in range(0,len(gps_files)):
        ...
        if len(gap_positions)>0:  #There are gaps
            for i in range(len(gap_positions)):
                if i==0:        
                    ...
                    #What is first epoch?
                    time=UTCDateTime('2017-09-23T00:00:00')+timedelta(seconds=t[0])-timedelta(seconds=18) #Apply GPS->UTC leapseconds
                    #Apply start time
...
if cut_filter:
    ...
    for k in range(len(stanames)):
        try:
            ...
            #Write to file
            n.write(path+'trim5Hz/'+sta+'.LXN.sac',format='SAC')
            e.write(path+'trim5Hz/'+sta+'.LXE.sac',format='SAC')
            u.write(path+'trim5Hz/'+sta+'.LXZ.sac',format='SAC')
...
if make_plots:
    ...
    for k in range(len(stanames)):
        ...
        try:
            ...
            n=read(path+'trim5Hz/'+sta+'.LXN.sac')
            e=read(path+'trim5Hz/'+sta+'.LXE.sac')
            z=read(path+'trim5Hz/'+sta+'.LXZ.sac')
            plt.figure()
            plt.subplot(311)
