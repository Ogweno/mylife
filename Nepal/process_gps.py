...
path='/Users/dmelgar/Nepal2015/GPS/'
        ...
        if len(gap_positions)>0:  #There are gaps
            for i in range(len(gap_positions)):
                if i==0:        
                    ...
                    #What is first epoch?
                    time=UTCDateTime('2015-04-25T00:00:00')+timedelta(seconds=t[0])-timedelta(seconds=16) #Apply GPS->UTC leapseconds
                    #Apply start time
...
if cut_filter:
    stanames=genfromtxt('/Users/dmelgar/Slip_inv/Nepal_ALOS/data/station_info/nepal_kinematic_gpsonly.gflist',usecols=0,dtype='S')
    coords=genfromtxt('/Users/dmelgar/Slip_inv/Nepal_ALOS/data/station_info/nepal_kinematic_gpsonly.gflist',usecols=[1,2])
    for k in range(len(stanames)):
        ...
        #Upsample to 5Hz
        if stanames[k]=='LHAZ':
            ti=arange(0,120.2,0.2)
            ni=interp(ti, n[0].times(), n[0].data, left=None, right=None)
            ei=interp(ti, e[0].times(), e[0].data, left=None, right=None)
            ui=interp(ti, u[0].times(), u[0].data, left=None, right=None)
            n[0].data=ni
            e[0].data=ei
            u[0].data=ui
            n[0].stats.delta=0.2
...
if move_mudpy:
    stanames=genfromtxt('/Users/dmelgar/Nepal2015/station_info/nepal.sta',usecols=0,dtype='S')
    for k in range(len(stanames)):
        copy(path+'cut/'+stanames[k]+'.LXN.sac','/Users/dmelgar/Slip_inv/Nepal_ALOS/data/waveforms/'+stanames[k]+'.disp.n')
        copy(path+'cut/'+stanames[k]+'.LXE.sac','/Users/dmelgar/Slip_inv/Nepal_ALOS/data/waveforms/'+stanames[k]+'.disp.e')
        copy(path+'cut/'+stanames[k]+'.LXZ.sac','/Users/dmelgar/Slip_inv/Nepal_ALOS/data/waveforms/'+stanames[k]+'.disp.u')
